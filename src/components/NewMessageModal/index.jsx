        ...
        <Modal show={show} onClose={cancelCreate} title={"Post a new message"} contentClass={"new_message_modal"}>
            <div className="emoji_picker">
                ...
                <div className={"emoji_options"}>
                    {Object.keys(EMOJI_MAP).map((emoji, desc) => (
                        <div key={desc}
                             className={`emoji_icon ${emoji === newMessageData.emoji ? 'selected_emoji' : ''}`}
                             onClick={() => updateMessageField('emoji', emoji)}
                             style={{background: EMOJI_MAP[emoji].color}}
            ...
            <div className="input">
                ...
                <input type="text" className="input_field"
                       placeholder={"The name of the person, you wish you said the words to"}
                       value={newMessageData.to_name}
                       onChange={(e) => updateMessageField('to_name', e.target.value)}/>
            </div>
            <div className="input">
                <p className="title">Message</p>
                <textarea className={"input_field"} rows={3}
                          placeholder={"The words, you wish you said when you had the chance"}
